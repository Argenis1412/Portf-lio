...
export default function Contact() {
  ...
  // Store error keys instead of full messages for reactive I18n
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus('success');
        setFormData({ nome: '', email: '', assunto: 'Contato via Portfólio', mensaje: '' } as any);
        // Special case for clearing after reset
        setFormData({ nome: '', email: '', assunto: 'Contato via Portfólio', mensaje: '', mensagem: '' } as any);
        // Correct way since keys might vary
        setFormData({
            nome: '',
            email: '',
            assunto: 'Contato via Portfólio',
